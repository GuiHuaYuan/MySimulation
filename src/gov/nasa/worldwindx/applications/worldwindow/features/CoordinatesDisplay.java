...
import static gov.nasa.worldwind.util.UnitsFormat.*;
...
public class CoordinatesDisplay extends AbstractOnDemandLayerFeature
{
    ...
    {
        super("坐 标", Constants.FEATURE_COORDINATES_DISPLAY,
    ...
    protected Layer doCreateLayer()
    {
        ...
        attrs.setFont(Font.decode("宋体-Bold-15"));
    ...
    private String formatText(DrawContext dc)
    {
        ...
        if (eyePosition != null) {
            ...
            // 英文替换中文
            units.setLabel(LABEL_DATUM, "坐标系:");
            units.setLabel(LABEL_LATITUDE, "纬度:");
            units.setLabel(LABEL_LONGITUDE, "经度:");
            //   units.setLabel(LABEL_LATLON_LAT, Logging.getMessage(LABEL_LATLON_LAT));
            //  units.setLabel(LABEL_LATLON_LON, Logging.getMessage(LABEL_LATLON_LON));
            units.setLabel(LABEL_HEADING, "航向：");
            units.setLabel(LABEL_EYE_ALTITUDE, "鹰眼高度:");
            units.setLabel(LABEL_PITCH, "倾角：");
            units.setLabel(LABEL_UTM_ZONE, "UTM区域:");
            units.setLabel(LABEL_UTM_EASTING, "东：");
            units.setLabel(LABEL_UTM_NORTHING, "北：");
            units.setLabel(LABEL_TERRAIN_HEIGHT, "地面高度：");
